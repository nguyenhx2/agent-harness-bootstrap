...
実プロジェクトに AI エージェントを載せたことがあれば、たぶん三つ以上は経験しているはずです。
どれも助言ではなく、このリポジトリの中の何かが答えます。

| | 起きること | 何が答えるか |
|---|---|---|
| 🔥 | *「機能を一つ頼んだら、3 モジュール 14 ファイルを触って `main` に force-push した」* | スコープを持つエージェント、パス単位のルール、そして終了コード 2 で push を着地前に拒否する**フック** |
| 🧠 | *「コンパクションが起きて、3 歩進んでいた計画を忘れた」* | タスクボードとセッションログはディスク上にあるので、新しいセッションが前回の停止地点から再開する |
| 🔑 | *「バグ修正のついでに `.env` を開いた」* | それらのパスは読み取りの前に拒否される。開くことを許されなかったものは漏らせない |
...
| | | |
|---|---|---|
| 📐 | **[`spec-builder`](spec-builder/)** | 思いつき、議事録、既存ドキュメントの山、空のリポジトリを、安定した要件 ID を持つ**一つの契約**に変えます。記述は国際標準に沿います。要件を勝手に作ることはなく、語られていないことは未解決事項として明示されます。[何ができるか](#-spec-builder-が作るもの) |
| 🏗️ | **[`harness-bootstrap`](harness-bootstrap/)** | まずコードを読み、それに**合う** `.claude/` ハーネスを構築します。担当範囲を持つエージェント、実在するパスにスコープされたルール、助言ではなく遮断するフック。[手に入るもの](#-手に入るもの) |
| 🔭 | **[`harness-view`](#-harness-view---任意のネイティブビューア)** | 結果を読み取って描画し、採点し、任意の部分を無効化できます。**モデルは介在しない**ので、ブラウザと CI が食い違うことはありません |
...
エージェントのキットは、たいてい全部入りで配られます。エージェントを全種類、スキルを百個、ルールも
フックもすべて、しかも誰かがコードベースを読む前に。気前がよく見えます。実際に起きているのは、
あなたが選んでいないチームを、見たこともないプロジェクトに割り当てることです。

これは見た目より高くつきます。

- **毎セッション、その全部にコンテキスト代を払う。** リポジトリのどのファイルにも一致しないルールは
  無料ではありません。以後すべてのリクエストにかかる税です。
- **持ち主のいない席が残る。** 誰も仕事を回さないエージェントは、予備の戦力ではありません。
  ルーティング表を読みにくくするだけの名前です。
- **助言が一般論になる。** どのリポジトリでも正しいように書かれた指針は、あなたのリポジトリで
  行動に移せるほど具体的であることは稀です。
- **どれが要なのか分からなくなる。** すべてが入っている状態では、何が選ばれて入ったのかを示す信号が
  なく、誰も安全に取り除けません。

網羅性が適合と取り違えられています。この二つは同じものではなく、対価を払う価値があるのは一方だけです。

<p align="center">
  <img src="docs/assets/tailored-build.ja.svg" alt="二つの半分からなる図。上半分は spec-builder と harness-bootstrap をつなぐ工程を示す。FR・NFR・BR の ID を持つ契約と、モジュール・検出された技術スタック・危険な操作からなるコードベース分析が、いずれも「テーラー」と名づけられた工程に入る。その仕事はカタログではなく証拠からチームを決めることである。そこからロースター（16席のうち7〜15席と、実在するモジュールごとの開発エージェント1名）、スキル（マニフェストが正当化するものだけ、選んだ場合に限る）、ルール（実在するパスにスコープ）が出て、ハーネスになる。下半分は、コードベースを読む前にすべてを入れてしまい全部にコンテキスト代を払い孤立した席と一般論の助言を抱えるキット方式と、契約と実在モジュールからロースターを導き導入物すべてに名前のついた担当がいるテーラーメイド構築とを比較する。" width="960">
</p>

### 真ん中にある工程

`spec-builder` は契約で終わります。`harness-bootstrap` はコードベースから始まります。**その間に
チームが決まる工程があり**、多くのキットにはこの工程自体が存在しません。

どちらの側も証拠です。契約は、番号のついた要件で「何を作るか」を語ります。コードベース分析は
「すでに何があるか」を語ります。モジュール、検出された技術スタック、危険な操作。ロースターは
この二つだけから導かれます。
...
<p align="center">
  <img src="docs/assets/harness-loom.ja.svg" alt="ハーネスを織機として描いた図。左では入力が実際に持ち込まれる形で並ぶ: 一行のアイデア、会議の議事録、既存のドキュメント、既存のリポジトリ、空のリポジトリ、書きかけの仕様書。もつれた糸がそれらを spec-builder へ運び、あらゆる形式が一つの契約に変換される。harness-bootstrap の枠の内側ではルールとフックがエージェントに降り、右のスキルレールがプロジェクトの技術スタックに合うスキルを見つけて必要なエージェントに配線する。下へ秩序立った三つの成果物が出ていき、枠の足元の状態がプロンプト・実行履歴・タスクボードを保持し、その下のハーネスビューアが状態を読んで制御面全体を表示・採点・切り替え・監視する" width="960">
</p>

織機はこの仕組みの正直な比喩です。糸はもつれたまま入り、布になって出てきます。魔法ではなく、
枠があるからそうなります。青い枠の内側はすべて枠組みです。`spec-builder` はその外側にあり、
仕事は一つだけ、持ち込まれたもの（思いつき、議事録、既存ドキュメント、空のリポジトリ）を
国際標準に沿った一つの契約に変えることです。内側ではルールとフックがエージェントに降り、
状態とスキルが左右から接続され、腰の部分から出てくるものは毎回同じ三つ、いずれも安定した
ID を持ちます。

この図の要点は色です。入る側では糸の色はばらばらで、平行なものは一本もありません。出る側では
一色になり、同じ方向を向いています。それがこのリポジトリの主張のすべてであり、モデルではなく
枠についての主張です。

### 織り目を見る

ハーネスは信用して受け入れる図ではありません。`harness-view` は
`.claude/state/harness-graph.json` から実物を描画し、どのノードも開けるファイルです。

<p align="center">
  <img src="docs/assets/harness-view-flow.png" alt="実プロジェクトの harness-view フロービュー: 左のルールがフックを経てエージェントの席に流れ込み、マージゲートに収束する" width="900">
</p>

<p align="center">
  <img src="docs/assets/harness-view-assess.png" alt="harness-view の Assess タブ: 実際のハーネスを 100 点満点中 79 点と採点し、ボード健全性・コスト管理・ドキュメント品質・安全性・トレーサビリティのカテゴリ別バーと、問題を名指しする検出結果一覧を表示" width="900">
</p>

いずれもモックではなく実在のハーネスです。ノード 111、エッジ 162、決定的な assess エンジンに
よる採点は 79/100。この採点にモデルは一切関与しないため、ブラウザと CI が食い違うことは
ありません。
...
概要図の中心的な着想は、マーベル・シネマティック・ユニバースの「時間織機（Temporal Loom）」から
得ています。もつれて発散した糸を、秩序ある検証可能なものへと織り上げる機械という発想です。
借りたのは着想のみで、マーベルの図版の複製・トレース・埋め込みは一切行っておらず、図はすべて
新規に描いています。参照:
[Temporal Loom](https://marvelcinematicuniverse.fandom.com/wiki/Temporal_Loom)（Marvel
Cinematic Universe Wiki）。マーベルおよびディズニーが本プロジェクトを推奨するものではありません。
...
ます。取得については **Hoàng Xuân Nguyên**（`nguyenhx2`）**nguyenhx1@gmail.com** までご連絡ください。
