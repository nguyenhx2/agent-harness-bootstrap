...
1. **The harness's safety properties do not depend on the model.** Proven, re-runnable, 33/33
   (66/66 across both hook flavors).
2. **The harness itself is cheap to install and cheap to carry, though less so than last cycle.**
   ...
   could recover it, see `benchmark/RESULTS.md`), 85% less to author, 66% of rule content kept out
