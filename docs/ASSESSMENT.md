...
| Advantage is the harness | Delivered, and falsifiable | `eval/guardrail_eval.py` scaffolds a harness and fires 21 payloads (11 must-block, 10 must-allow) at it: 21/21 correct. Every block is a shell script and an exit code. Swap Opus for Haiku and the result is byte-identical. The safety floor is model-independent, and the claim is re-runnable. |
| ROI | Half delivered | Cost is modelled per roster profile (`benchmark/model_cost.py`) and the harness's own overhead is measured (`benchmark/RESULTS.md`: read path -62%, write path -85%). Value is not measured at all. Cost-per-feature without value-per-feature is not ROI. |
...
1. **The harness's safety properties do not depend on the model.** Proven, re-runnable, 21/21.
2. **The harness itself is cheap to install and cheap to carry.** Measured: 62% less to read, 85%
