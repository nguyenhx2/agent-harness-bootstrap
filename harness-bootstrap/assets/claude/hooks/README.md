...
| `graph-stale` | PostToolUse | `Edit\|Write` | Nothing. Appends every edited source file to `.claude/state/code-graph.stale` when a code graph exists, so `/code-graph --check` and `/board-audit` see the graph lagging the code. | Non-blocking, always exit 0. Rebuild is deliberate (`/code-graph`), never a side effect of an edit. |
