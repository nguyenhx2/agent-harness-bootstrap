...
- **S - script / CLI / library, single domain (5-6 agents).** `orchestrator`, `app-dev`, `qa-test`
  (if tests are chosen), `reviewer` (the merged gate, flag `solo_review` - the recommended shape
  here; split reviewers remain the alternative), `spec-guardian`. **This is the floor** - below it
  the standard feature flow has missing seats.
