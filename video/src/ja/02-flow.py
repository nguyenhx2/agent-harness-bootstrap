...
class Flow(Scene):
    def construct(self):
        ...
        # ---- title card -----------------------------------------------
        t = title_text("運用の流れ", fs=46, color=WHITE)
        legend = VGroup(
            tag("緑 = 無料・決定的", GREEN_HI, fs=20),
            tag("紫 = 課金されるトークン", PURPLE_HI, fs=20),
            tag("赤 = ブロック", RED, fs=20),
        ...
        specs_sub = Text("契約書", font=JFONT, font_size=22, color=DIM).next_to(specs, DOWN, buff=0.22)
        a1 = Arrow(sb.get_right(), specs.get_left(), buff=0.2, color=DIM, stroke_width=3)

        self.play(GrowFromCenter(sb), run_time=0.5)
        caption(self, "spec-builderがヒアリングし、仕様一式 - 契約書を書く。", hold=1.1, y=-3.5, size=24)
        ...
        steps = [
            ("あなたのコードを読む", PURPLE_HI, PURPLE),
            ("スコープを絞ったロースターを選ぶ", PURPLE_HI, PURPLE),
            ("約0.2秒でスキャフォルドする", GREEN_HI, GREEN),
            ("オーケストレーションを配線する", GREEN_HI, GREEN),
        ]
        step_chips = VGroup()
        for label, stroke, fill in steps:
            c = chip(label, fill, stroke, fs=17, h=0.72, w=2.95)
            step_chips.add(c)
        step_chips.arrange(RIGHT, buff=0.28).move_to([0, -0.6, 0])

        self.play(GrowFromCenter(hb), run_time=0.5)
        caption(self, "harness-bootstrapがコードを読み、ハーネスをスキャフォルドする。", hold=0.8, y=-3.5, size=24)
        for c in step_chips:
        ...
        head = Text("その後、タスクループが回る", font=JFONT, font_size=28, color=WHITE).move_to([0, 1.6, 0])
        self.play(FadeOut(row2), FadeIn(head), run_time=0.6)

        orch = chip("オーケストレーター", PURPLE, PURPLE_HI, fs=22, h=0.9, w=3.4).move_to([0, 0.55, 0])
        dev = chip("スコープを絞った\n専門エージェント", PURPLE, PURPLE_HI, fs=18, h=1.0, w=3.4).move_to([-3.6, -1.4, 0])
        board = chip("ディスク上の\nタスクボード", NEUTRAL, WHITE, fs=18, h=1.0, w=3.0).move_to([3.6, -1.4, 0])
        ...
        d_label = Text("起動する", font=JFONT, font_size=18, color=DIM).next_to(dispatch, LEFT, buff=0.05).shift(UP*0.1)
        r_label = Text("記録する", font=JFONT, font_size=18, color=DIM).next_to(record, UP, buff=0.05)
        self.play(Create(dispatch), FadeIn(d_label), run_time=0.5)
        caption(self, "オーケストレーターがスコープを絞った専門エージェントを起動する。", hold=0.9, y=-3.5, size=24)
        ...
        hook_t = Text("フック", font=JFONT, font_size=22, color=WHITE).move_to(hook.get_center())
        ...
        blocked = Text("ブロック", font=JFONT, font_size=20, color="#FFBA08").next_to(hookg, RIGHT, buff=0.3)
        ...
        caption(self, "フックは危険な操作を、実行される前にブロックする。", hold=1.0, y=-3.5, size=24)

        # crash + resume
        self.play(FadeOut(VGroup(orch, dev, dispatch, record, d_label, r_label, hookg, bad, blocked)), run_time=0.5)
        crash = Text("エージェントがクラッシュ - コンテキスト消失", font=JFONT, font_size=24, color=RED).move_to([0, 0.4, 0])
        self.play(FadeIn(crash), Indicate(board, color=GREEN_HI, scale_factor=1.15), run_time=0.9)
        self.wait(0.5)
        fresh = chip("新しいエージェント", PURPLE, PURPLE_HI, fs=20, h=0.85, w=3.0).move_to([-3.6, -1.4, 0])
        ...
        caption(self, "ボードはクラッシュを生き延びる - 新しいエージェントが止まった場所から再開する。", hold=2.2, y=-3.5, size=22)
        self.wait(0.4)
